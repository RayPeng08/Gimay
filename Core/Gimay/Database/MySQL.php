...
use Gimay;
class MySQL implements \Gimay\IDatabase
{
    public $debug = false;
    public $conn = null;
    public $config;
    ...
    function connect()
    {
        ...
        if (!$this->conn)
        {
            Gimay\Error::info(__CLASS__." SQL Error", mysql_error($this->conn),701);
            return false;
        }
        mysql_select_db($db_config['name'], $this->conn) or Gimay\Error::info("SQL Error", mysql_error($this->conn),702);
        if ($db_config['setname'])
        {
            mysql_query('set names ' . $db_config['charset'], $this->conn) or Gimay\Error::info("SQL Error",
                mysql_error($this->conn),702);
        }
        return true;
    }
    ...
    {
        return mysql_error($this->conn) . "MySQL Server: {$this->config['host']}:{$this->config['port']}";
    }
    ...
    function query($sql)
    {
        ...
        for ($i = 0; $i < 2; $i++)
        {
            ...
            if ($res === false)
            {
                ...
                \Gimay\Error::info(__CLASS__." SQL Error", $this->errorMessage($sql),701);
                return false;
            }
            break;
        }
...
class MySQLRecord implements \Gimay\IDbRecord
{
    public $result;
