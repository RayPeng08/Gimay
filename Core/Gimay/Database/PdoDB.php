...
use Gimay;
class PdoDB extends \PDO implements Gimay\IDatabase
{
    public $debug = false;
    protected $config;
    ...
    public final function query($sql)
    {
        ...
        $res = parent::query($sql) or \Gimay\Error::info(
            "SQL Error",
            implode(", ", $this->errorInfo()) . "<hr />$sql",703
        );
    ...
    public final function queryLine($sql, $_)
    {
        ...
        {
            \Gimay\Error::info("SQL Error", implode(", ", $this->errorInfo()) . "<hr />$sql",703);
            return false;
        }
    }
    ...
    public final function execute($sql, $_)
    {
        ...
        {
            \Gimay\Error::info("SQL Error", implode(", ", $this->errorInfo()) . "<hr />$sql",704);
            return false;
        }
    }
