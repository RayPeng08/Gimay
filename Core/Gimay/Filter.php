...
class Filter
{
    ...
    {
        if (self::$error_url) {
            \Gimay::$php->http->redirect(self::$error_url);
        }
        if ($text) {
            exit($text);
        } else {
    ...
    static function safe(&$content)
    {
        ...
        $content = html_entity_decode($content, ENT_QUOTES, \Gimay::$charset);
    }
    ...
    public static function escape($string)
    {
        ...
        //HTML转义
        $string = htmlspecialchars($string, ENT_QUOTES, \Gimay::$charset);
        //启用了magic_quotes
