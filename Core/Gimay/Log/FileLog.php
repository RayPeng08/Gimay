 ...
 */
namespace Gimay\Log;
class FileLog extends \Gimay\Log implements \Gimay\IFace\Log
{
    protected $log_file;
    protected $log_dir;
    protected $fp;
    ...
    function __construct($config)
    {
        ...
        if ($this->archive) {
            if (isset($config['dir'])) {
                ...
                throw new \Exception(__CLASS__ . ": 日志存储目录不存在 \$config['dir']");
            }
        } else {
            if (isset($config['file'])) {
                $this->log_file = $config['file'];
            } else {
                throw new \Exception(__CLASS__ . ": 配置文件不存在 \$config[file]");
            }
        }
        ...
        if (file_exists($dir)) {
            if (!is_writeable($dir) && !chmod($dir, 0777)) {
                throw new \Exception(__CLASS__ . ": {$dir} 目录不可写.");
            }
        } elseif (mkdir($dir, 0777, true) === false) {
            throw new \Exception(__CLASS__ . ": 创建目录 {$dir} 失败.");
        }
...
//            }
    ...
    private function openFile($file)
    {
        ...
        if (!$fp) {
            throw new \Exception(__CLASS__ . ": 无法打开日志文件[{$file}].");
        }
