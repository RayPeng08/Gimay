...
class HttpServer extends Gimay\Protocol\WebServer implements Gimay\IFace\Protocol
{
    ...
    function __construct($config = array())
    {
        ...
        $mimes = require(CORE_PATH . '/Data/Mimes.php');
        $this->mime_types = array_flip($mimes);
        $this->config = $config;
        $this->parser = new Gimay\Http\Parser;
    }

    function onStart($serv, $worker_id = 0)
    {
        if (!defined('WEBROOT')) {
            $webroot = empty($this->config['server']['outhost']) ? $this->config['server']['host'] : $this->config['server']['outhost'];
            $webroot .= ':' . $this->config['server']['port'];
            define('WEBROOT', $webroot);
        }

        if (isset($this->config['server']['user'])) {
            Gimay\Console::changeUser($this->config['server']['user']);
        }

        Gimay\Error::$echo_html = true;
        $this->swoole_server = $serv;
        Gimay::$php->server = $this;
    ...
    function onReceive($serv, $client_id, $from_id, $data)
    {
        ...
        if ($response and $response instanceof Gimay\Response) {
            //发送response
            $this->response($request, $response);
        }
    }

    function afterResponse(Gimay\Request $request, Gimay\Response $response)
    {
     ...
     * 发送响应
     * @param $request Gimay\Request
     * @param $response Gimay\Response
     * @return bool
     */
    function response(Gimay\Request $request, Gimay\Response $response)
    {
     ...
     * 发生了http错误
     * @param                 $code
     * @param Gimay\Response $response
     * @param string $content
     */
    function httpError($code, Gimay\Response $response, $content = '')
    {
        $response->setHttpStatus($code);
        $response->head['Content-Type'] = 'text/html';
        $response->body = Gimay\Error::info(Gimay\Response::$HTTP_HEADERS[$code], "[$content]" . self::SOFTWARE . " 位于 {$this->server->host} 端口 {$this->server->port}", 601);
    }
    ...
    private function errorResponse($error)
    {
        ...
        $message = Gimay\Error::info(self::SOFTWARE . "系统错误", $errorMsg, 101);
        if (empty($this->currentResponse)) {
            $this->currentResponse = new Gimay\Response();
        }
     ...
     * 处理请求
     * @param $request
     * @return Gimay\Response
     */
    function onRequest(Gimay\Request $request)
    {
        $response = new Gimay\Response;
        $this->currentResponse = $response;
        \Gimay::$php->request = $request;
        \Gimay::$php->response = $response;
        ...
        elseif (isset($this->dynamic_ext[$request->ext_name]) or empty($ext_name)) {
            ...
            $this->httpError(404, $response, "服务请求不存在({($request->meta['path']})");
        }
        return $response;
    }
    ...
    function doStaticRequest($request, $response)
    {
        ...
        //扩展名
        $request->ext_name = $ext_name = Gimay\Upload::getFileExt($request->meta['path']);
        /* 检测是否拒绝访问 */
     ...
     * @return unknown_type
     */
    function processStatic($request, Gimay\Response $response)
    {
        ...
        if (is_file($path)) {
            ...
            $ext_name = Gimay\Upload::getFileExt($request->meta['path']);
            if ($read_file) {
     ...
     * @return unknown_type
     */
    function processDynamic(Gimay\Request $request, Gimay\Response $response)
    {
        $path = $this->document_root . '/' . $request->meta['path'];
        if (is_file($path)) {
            $request->setGlobal();
