...
class Redis
{
    ...
    static function getIncreaseId($appKey, $init_id = 1000)
    {
        ...
        //已存在 就加1
        if (\Gimay::$php->redis->exists($main_key)) {
            $inc = \Gimay::$php->redis->incr($main_key);
            if (empty($inc)) {
                \Gimay::$php->log->put("redis::incr() failed. Error: " . \Gimay::$php->redis->getLastError());
                return false;
            }
            return $inc;
        } //上面的if条件返回false,可能是有错误，或者key不存在，这里要判断下
        else if (\Gimay::$php->redis->getLastError()) {
            return false;
        } //这才是说明key不存在，需要初始化
        else {
            $init = \Gimay::$php->redis->set($main_key, $init_id);
            if ($init == false) {
                \Gimay::$php->log->put("redis::set() failed. Error: " . \Gimay::$php->redis->getLastError());
                return false;
            } else {
                return $init_id;
            }
    ...
    function connect()
    {
        try {
            ...
            \Gimay::$php->log->error(__CLASS__ . " Redis Exception" . var_export($e, 1));
            return false;
        }
    }
    ...
    function __call($method, $args = array())
    {
        ...
        while (1) {
            try {
                ...
                \Gimay::$php->log->error(__CLASS__ . " [" . posix_getpid() . "] Redis[{$this->config['host']}:{$this->config['port']}]
