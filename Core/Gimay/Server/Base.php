...
abstract class Base implements Driver
{
    ...
    {
        if (!($this->protocol instanceof Gimay\Network\Server))
        {
            throw new \Exception("当前不为 Gimay\\Network\\Server 实例,不能调用addListener方法.");
        }
    }
    ...
    {
        if (!($protocol instanceof Gimay\IFace\Protocol))
        {
            throw new \Exception("当前不为 \\Gimay\\IFace\\Protocol 实例,不能调用setProtocol方法.");
        }
    ...
    function accept()
    {
        ...
        $client_socket_id = intval($client_socket);
        ...
        {
            Gimay\Network\Stream::close($client_socket);
            return false;
        }
        else
        {
    ...
    function daemonize()
    {
        ...
        {
            throw new \Exception(__METHOD__ . ": pcntl_fork 方法找不到.");
        }
        $pid = pcntl_fork();
        if ($pid == -1)
        {
    ...
    {
        return Gimay\Network\Stream::write($sock, $data);
    }
