...
class Upload
{
    ...
    function __construct($config)
    {
        if (empty($config['base_dir']) or empty($config['base_url'])) {
            throw new \Exception(__CLASS__ . ' 上传目录和本地目录域名不能为空.');
        }
    ...
    function save($name, $filename = null, $allow = null)
    {
        ...
        if ($filetype === false) {
            $this->error_msg = "无法识别 '$mime' 文件格式!";
            $this->error_code = 1;
            return false;
        } elseif (!in_array($filetype, $this->allow)) {
            $this->error_msg = "文件类型 '$filetype' 不允许上传!";
            $this->error_code = 2;
            return false;
        }
        ...
        if ($filename === null) {
            ...
            $this->error_msg = "文件 '$path/$filename.$filetype' 已存在!";
            $this->error_code = 3;
            return false;
        }
        ...
        if (self::moveUploadFile($_FILES[$name]['tmp_name'], $save_filename)) {
            ...
            $this->error_msg = "移动文件失败. tmp_name={$_FILES[$name]['tmp_name']}|dest_name={$save_filename}";
            $this->error_code = 2;
            return false;
        }
    }
